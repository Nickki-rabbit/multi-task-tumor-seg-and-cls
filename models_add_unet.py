...
#######################################
#######################################
class conv_block(nn.Module): # CBR
    def __init__(self, ch_in, ch_out):
        super(conv_block, self).__init__()
        self.conv = nn.Sequential(
            nn.Conv2d(ch_in, ch_out, kernel_size=7, stride=2, padding=3, bias=False),
...
class up_conv(nn.Module):   # C'BR
    def __init__(self, ch_in, ch_out):
        super(up_conv, self).__init__()
        self.up = nn.Sequential(
            # nn.Upsample(scale_factor=2),
            # nn.Conv2d(ch_in, ch_out, kernel_size=3, stride=1, padding=1, bias=False),
            nn.ConvTranspose2d(ch_in, ch_out, kernel_size=7, stride=2, padding=3, output_padding=1, bias=False),
            nn.BatchNorm2d(ch_out),
...
class Y_Net(nn.Module):
    ...
    def __init__(self, n_classes, diagClasses, channel_size):
        super(Y_Net, self).__init__()
        # self.input_channels = 448
        # self.patch_size = 64
        
        self.Conv1 = conv_block(ch_in=channel_size, ch_out=64)
        self.Conv2 = conv_block(ch_in=64, ch_out=128)
        self.Conv3 = conv_block(ch_in=128, ch_out=256)
        
        #self.Conv1 = nn.Conv2d(448, 64, kernel_size=(3,3), stride=1, padding=1)
        #self.bn1 = nn.BatchNorm2d(64)
        #self.relu1 = nn.ReLU(True)
        #self.Conv2 = nn.Conv2d(64, 128, kernel_size=(5, 5), stride=1, padding=2)
        #self.Conv3 = nn.Conv2d(128, 256, kernel_size=(7, 7), stride=1, padding=3)
        #self.bn2 = nn.BatchNorm2d(128)
        #self.relu2 = nn.ReLU(True)
        #self.bn3 = nn.BatchNorm2d(256)
        #self.relu2 = nn.ReLU(True)
        #self.Up3 = nn.ConvTranspose2d(256, 128, kernel_size=(3,3), stride=1, padding=1, output_padding=0, bias=False)
        #self.Up2 = nn.ConvTranspose2d(256, 64, kernel_size=(5,5), stride=1, padding=2, output_padding=0, bias=False)
        #self.Up1 = nn.COnvTranspose2d(128, n_classes, kernel_size=(7,7), stride=1, paddind=3, output_paddind=0, bias=False)
        ...
        self.Up1 = up_conv(ch_in=128, ch_out=n_classes)
    ...
    def forward(self, x):
        # x = x.squeeze() 
        # encode
        x1 = self.Conv1(x) # 448->64
        x2 = self.Maxpool(x1)
        x2 = self.Conv2(x1) # 64 ->128
        x3 = self.Maxpool(x2)
        x3 = self.Conv3(x2) # 128->256

        # cls branch
        glb_avg = self.global_avg(x3)
        flatten = glb_avg.view(glb_avg.size(0), -1)
        ...
        d3 = torch.cat((x2, d3), dim=1) # channel N * C * H * W ???    
        # d3 = self.Up_conv3(d3)
        d2 = self.Up2(d3)
