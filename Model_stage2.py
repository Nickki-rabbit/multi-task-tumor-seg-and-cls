...
class PSPDec(nn.Module):
    def __init__(self, nIn, nOut, downSize, upSize=8): # 48 to 8
        super().__init__()
...
class ResNetC1(nn.Module):
    ...
    def __init__(self, classes):
        super().__init__()
        self.level1 = CBR(448, 16, 7, 2)  # 384 x 384

        self.p01 = PSPDec(16 + classes, classes, 160, 32)
        self.p02 = PSPDec(16 + classes, classes, 128, 32)
        self.p03 = PSPDec(16 + classes, classes, 96, 32)
        self.p04 = PSPDec(16 + classes, classes, 72, 32)
        ...
        self.p10 = PSPDec(classes + 256, 64, 80, 16)
        self.p20 = PSPDec(classes + 256, 64, 64, 16)
        self.p30 = PSPDec(classes + 256, 64, 48, 16)
        self.p40 = PSPDec(classes + 256, 64, 36, 16)

        self.class_1 = nn.Sequential(
            nn.Conv2d(classes + 256 + 64 * 4, classes, 3, padding=1, bias=False),
            nn.BatchNorm2d(classes, momentum=0.95, eps=1e-3),
            nn.ReLU(True),  # nn.PReLU(classes),
            # nn.Dropout2d(.1),
