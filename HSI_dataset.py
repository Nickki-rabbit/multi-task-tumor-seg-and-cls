...
class LoadDatasetName:
    def __init__(self, data_dir, classes):
        ...
        self.classWeight = np.ones(self.classes, dtype=np.float32)
...
class HSI_Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, imList, gtList):
        self.imList = imList
        self.gtList = gtList
    ...
    def cut_random_patches(self, data, gt):
        ...
        # to categorical in keras -> convert to one-hot
        return patches, patches_gt
    
    def cut_patches(self, data, gt):
        patches = []
        patches_gt = []
        
        for i in range(0, data.shape[0], self.patch_size//2):
            for j in range(0, data.shape[1], self.patch_size//2):
                patch = (data[i:i+self.patch_size, j:j+self.patch_size, :]).copy()
                patch_gt = (gt[i:i+self.patch_size, j:j+self.patch_size]).copy()
                patches.append(patch)
                patches_gt.append(patch_gt)
        patches = np.array(patches)
        patches_gt = np.array(patches_gt)
        print('img cropped.patch_shape:', np.shape(patches))
        return patches, patches_gt                
    ...
    def __getitem__(self, idx):
        ...
        # print('the index right now is: ', idx)
        # patch_img = []
        
        gt_name = self.gtList[idx]
        img_mat = loadmat('./data_file/specimen.mat')[image_name]
        gt_mat = loadmat('./data_file/specimen_gt.mat')[gt_name]
        patch_img_mat, patch_gt_mat = self.cut_random_patches(img_mat, gt_mat) # [30, 16, 16, 448] per hsi image
        
        # patch_img_mat, patch_gt_mat = self.cut_patches(img_mat, gt_mat)
