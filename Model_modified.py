...
class CBR(nn.Module): # conv block contains: conv->BN->ReLU
...
class PSPDec(nn.Module):
    ...
    '''
    def __init__(self, nIn, nOut, downSize, upSize=8):
        super().__init__()
...
class ResNetC1(nn.Module):
    ...
    def __init__(self, classes):
        ...
        self.patch_size = 64
        self.level1 = CBR(448, 16, 7, 2)

        self.p01 = PSPDec(16 + classes, classes, 160, self.patch_size//2) # 192 to patch_size//2
        self.p02 = PSPDec(16 + classes, classes, 128, self.patch_size//2)
        self.p03 = PSPDec(16 + classes, classes, 96, self.patch_size//2)
        self.p04 = PSPDec(16 + classes, classes, 72, self.patch_size//2)
        ...
        self.p10 = PSPDec(3 + 256, 64, 80, self.patch_size//4) # 8 to 3, 96 to patch_size//4
        self.p20 = PSPDec(3 + 256, 64, 64, self.patch_size//4)
        self.p30 = PSPDec(3 + 256, 64, 48, self.patch_size//4)
        self.p40 = PSPDec(3 + 256, 64, 36, self.patch_size//4)

        self.class_1 = nn.Sequential(
            nn.Conv2d(3 + 256 + 64 * 4, classes, 3, padding=1, bias=False), # 8 to 3
            nn.BatchNorm2d(classes, momentum=0.95, eps=1e-3),
            nn.ReLU(True),
