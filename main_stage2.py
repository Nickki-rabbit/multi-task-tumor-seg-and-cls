...
import HSI_dataset_stage2 as myDataLoader
import time
...
def train(args, train_loader, model, criterion, criterion1, optimizer, epoch):
    ...
    for i, (input, target, target2) in enumerate(train_loader):
        ...
        bs, patch, c, w, h = input.size() # 4x30x448x64x64
        input = input.view(-1, c, w, h) # ==> 120 x 448 x 64 x 64
        target = target.view(-1, w, h) # 4,30,64,64 ==> 120,64,64
        target2 = target2.view(-1, 1) # 4,30,1 ==> 120x1
...
def trainValidateSegmentation(args):
    ...
    if not os.path.isfile(args.cached_data_file):
        ...
        dataLoader = myDataLoader.LoadDatasetName(args.data_dir, args.classes, args.diagClasses)
        if dataLoader == None:
    ...
    trainLoader = torch.utils.data.DataLoader(
                        myDataLoader.HSI_Dataset(data['trainIm'], data['trainAnnot'], data['trainDiag']),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--inWidth', type=int, default=16, help='Width of the input patch')
    parser.add_argument('--inHeight', type=int, default=16, help='Height of the input patch')
    ...
    assert args.modelType in ['C1', 'D1', 'U1', 'Y1']
